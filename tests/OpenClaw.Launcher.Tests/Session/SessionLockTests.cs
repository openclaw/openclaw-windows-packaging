...
public sealed class SessionLockTests
{
    ...
    public void AbandonedLockIsRecoveredRatherThanDeadlocking()
    {
        ...
        // A thread that exits without releasing stands in for a process that
        // crashed mid-transition; the next caller must still make progress.
        var abandoner = new Thread(() => sessionLock.TryAcquire(TimeSpan.Zero));
        abandoner.Start();
