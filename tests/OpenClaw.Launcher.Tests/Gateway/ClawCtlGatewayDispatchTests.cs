...
public sealed class ClawCtlGatewayDispatchTests : IDisposable
{
        ...
        params string[] args)
    {
        ...
        int exitCode = await Program.RunControlAsync(
            ...
            errors.Add,
            output,
            createGatewayRuntime: CreateRuntime);
    ...
    public async Task StatusIsReadOnly()
    {
        ...
        Assert.Contains("No gateway has been started", output);
    ...
    public async Task InstallStartsTheGatewayAndConfiguresSignInRecovery()
    {
        ...
        Assert.Contains(
            _scheduler.Calls,
            call => call.StartsWith("register:", StringComparison.Ordinal));
        Assert.Contains("Sign-in recovery", output);
    }
    ...
    public async Task StatusReportsARunningGatewayAfterInstall()
    {
        ...
        Assert.Contains("Sign-in recovery: configured", output);
    }
    ...
    [Fact]
    public async Task ABareNounReportsUsageAndChangesNothing()
    {
        (int exitCode, _, string error) = await RunAsync("gateway-service");

        Assert.Equal(2, exitCode);
        Assert.Contains("requires a sub-command", error);
        Assert.Empty(_client.Calls);
