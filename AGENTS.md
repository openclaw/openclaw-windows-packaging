...
- For the fast loose-registration loop, use `scripts\Deploy-LocalPackage.ps1 -Patch <name>` with a name that describes the work, such as `-Patch pwsh-exec`, so iteration registers beside the base `openclaw` and `clawctl` instead of replacing them. Deploy without `-Patch` only when the task exercises the base identity itself: its aliases, shell completion, or a loose/MSIX transition. Use `scripts\Build-LocalMSIX.ps1` for a real package. Loose registration and installed MSIX state are mutually exclusive for one identity, and switching can delete packaged app data.
- Before finishing, tear down and unregister each patch this task deployed unless the user asks to keep it: run `clawctl-<name> teardown --force` while the alias still exists, then `scripts\Deploy-LocalPackage.ps1 -Unregister -Patch <name>` with the same `-Architecture`. Unregistering first leaves the isolated session behind until the same patch is redeployed.
