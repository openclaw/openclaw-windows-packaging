...
| `clawctl completion` | Write the PowerShell completion script for `clawctl` to standard output. Source it for the current shell, or use `--install` to add a marked block to the current-user PowerShell profile. |
| `clawctl completion --install [--profile <path>]` | Atomically update the selected profile (or the current-user PowerShell profile) and cache upstream `openclaw` completion in host LocalState. `clawctl pwsh` projects that cache into the current isolated workspace. If setup is unavailable, the host completion still installs and reports the unavailable agent cache as a warning. `--uninstall` removes the marked profile block and invalidates the host cache. |
