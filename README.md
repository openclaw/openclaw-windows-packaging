...
| `clawctl completion --install [--profile <path>]` | Atomically update the selected profile (or the current-user PowerShell profile) and capture upstream `openclaw` completion into the isolated agent workspace for later `clawctl pwsh` sessions. If setup is unavailable, the host completion still installs and reports the unavailable agent cache as a warning. Use `--uninstall` instead to remove only the marked profile block. |
