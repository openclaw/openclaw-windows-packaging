...
| `clawctl completion --install [--profile <path>]` | Atomically update the selected profile (or the current-user PowerShell profile) with the host completer and the immutable upstream script generated while building the pinned payload. The trusted upstream script is cached in host LocalState, and `clawctl pwsh` safely projects it into the current isolated workspace. `--uninstall` removes the marked profile block and invalidates the host cache so later agent shells do not load a stale projection. |
