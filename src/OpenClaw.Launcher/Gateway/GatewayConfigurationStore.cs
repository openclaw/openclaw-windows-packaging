...
/// find it.
/// </remarks>
public sealed record GatewayLaunchConfiguration
{
    /// <summary>
    /// The default port. Recorded explicitly on first use, so a later change to
    /// this constant cannot silently move an existing installation's gateway.
    /// </summary>
...
public enum GatewayConfigurationFault
{
...
/// </summary>
public sealed class GatewayConfigurationStore
{
    ...
    public GatewayConfigurationResult Read()
    {
        ...
        if (!IsUsablePort(configuration.Port))
        {
            return new GatewayConfigurationResult(
                null,
                GatewayConfigurationFault.Invalid,
                $"The configured port {configuration.Port} is not a usable " +
                "TCP port (1-65535).");
        }
    ...
    public void Write(GatewayLaunchConfiguration configuration)
    {
        ...
        {
            throw new GatewayConfigurationException(
                $"The port {configuration.Port} is not a usable TCP port (1-65535).");
        }
...
/// </summary>
public sealed class GatewayConfigurationException(string message)
    : Exception(message);
